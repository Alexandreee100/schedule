...
import { Reference, type ReferenceProps } from "src/shared/lib/floating/reference";
...
export const createFloatingComponents = <TProps,>(
    ...
    const context = createContext<(UseFloatingReturn & UseInteractionsReturn) | null>(null);
    ...
    const ReferenceComponent = <T extends ReferenceType>(props: ReferenceProps<T>) => {
        return <Reference useContextHook={useContextHook} {...props} />;
    };

    const FloatingComponent = <T extends ReferenceType>(props: FloatingProps<T>) => {
        return <Floating useContextHook={useContextHook} {...props} />;
    };

    return [Provider, ReferenceComponent, FloatingComponent, useOpenState, useContextHook] as const;
};
