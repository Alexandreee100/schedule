...
export function useControlledValue<T, R extends unknown[] = unknown[]>(
    ...
    const enableWarning = props.enableWarning ?? false;

    useWarning(enableWarning, isControlled);
    ...
    const setValue = useCallback(
        (value: T | SetStateAction<T>, ...rest: R) => {
            ...
            if (isControlled) {
                const nextValue = isFunction(value)
...
const useWarning = (isEnabled: boolean, isControlled: boolean) => {
    ...
    }, [isControlled, isEnabled]);
};

const isFunction = <T>(action: SetStateAction<T>): action is (prev: T) => T =>
