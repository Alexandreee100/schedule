...
export const isCsvQueryParamValue = (
    value: QueryParamValue,
): value is CsvQueryParamValue =>
    typeof value === "object" &&
    value !== null &&
    !Array.isArray(value) &&
    value.format === "csv";

export const formatCsvQueryValue = (
    value: QueryValue[],
    options?: AddQueryParamsOptions,
): string | undefined => {
    const queryValues = value.filter((item) =>
        shouldAppendQueryValue(item, options),
    );
