...
export const createQueryApi = (queryClient: AnyQueryClient) => {
    ...
    >(
        config: () => QueryOptionsParams<TQueryFnData, TError, TData, TQueryData, TQueryKey>
    ...
    const createInfiniteQuery = <
        ...
        TQueryKey extends QueryKey = QueryKey,
    >(
        config: () => InfiniteQueryFlattenConfig<TQueryFnData, TError, TPageParam, TData, TQueryKey>
    ) => new InfiniteQuery(queryClient, config);

    const createMutation = <TData = unknown, TVariables = void, TError = DefaultError, TOnMutateResult = unknown>(
        config: Omit<MutationConfig<TData, TVariables, TError, TOnMutateResult>, "queryClient">
    ) => {
