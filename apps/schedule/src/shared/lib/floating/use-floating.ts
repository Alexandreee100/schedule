...
export interface UseFloatingReturn<
    TReferenceType extends ReferenceType = ReferenceType,
...
const useMiddleware = ({
    ...
    FloatingProps,
    "disableDefaultMiddleware" | "offsetOptions" | "middleware"
>) => {
    const offsetNumber =
        initialOffsetOptions && typeof initialOffsetOptions === "number"
            ? initialOffsetOptions
            : 4;

    const defaultMiddleware: UseFloatingOptions["middleware"] =
        disableDefaultMiddleware
            ? []
            : [
                  offset(2),
