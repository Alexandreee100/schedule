...
export const useGridTableAdapter = <TData extends RowData>(table: Table<TData>, totalWidth: number | undefined) => {
	...
	const rows: IRowGridTable[] = useMemo(
		() =>
			tableRows.map((row) => {
				const cells = row.getVisibleCells().map((cell): ICellGridTable => {
					return {
						id: cell.id,
