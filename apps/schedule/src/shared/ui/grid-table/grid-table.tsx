...
export const GridTable = forwardRef<HTMLDivElement, IGridTableProps>(
	({ columnSizes, rows, headerRows, rowHeight, className, density, dividers }, ref) => {
		const tableStyle: CSSProperties | undefined =
			rowHeight !== undefined ? { "--row-height": `${rowHeight}px` } : undefined;
		...
		return (
			...
			>
				...
				{rows.map((row) => (
					<Grid key={row.id} columns={gridTemplateColumns} className={styles.row}>
						{row.cells.map((cell) => (
							<Flex
								key={cell.id}
								className={styles.cell}
